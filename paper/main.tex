...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/larp_before_after_hero.png}
  \caption{\textbf{LARP-Scaler image upscaling.} A real $512\times512$ input
  (left) and the $2048\times2048$ output of the released $4\times$
  LARP-Scaler checkpoint (right).}
  \label{fig:larp_before_after}
\end{figure*}

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/pid_lua_image_upscale_examples.png}
  \caption{\textbf{Image-upscaling examples in valid operating regimes.}
  Native PiD is evaluated at $512\!\to\!2048$, while LUA-FLUX with the Flux
  VAE is evaluated at $256\!\to\!1024$; both rows use real images encoded by
  the VAE, followed by latent upscaling and final decoding. The rows are
  illustrative examples, not a cross-resolution metric comparison.}
  \label{fig:pid_lua_image_upscale_examples}
\end{figure*}
...
dB for Real-ESRGAN and 19.554 dB for LUA. On 12 anime
...
\begin{table}[t]
  ...
  \texttt{piq}. PiD is omitted here because its archived 512-pixel outputs are
  out of regime; LUA per-image outputs were not archived.}
  ...
  \begin{tabular}{llcc}
    ...
    \midrule
    Photo & \method & 0.4221 & \textbf{0.1766}\\
          & Real-ESRGAN & \textbf{0.3778} & 0.1950\\
    \midrule
    Anime & \method & 0.2624 & \textbf{0.1215}\\
          & Real-ESRGAN & \textbf{0.1537} & \textbf{0.1215}\\
    \bottomrule
  \end{tabular}
\end{table}

\Cref{tab:perceptual} complements distortion metrics. \method leads on DISTS
for photographs and matches Real-ESRGAN on anime, while Real-ESRGAN has lower
LPIPS on both sets. A distortion-oriented lead thus does not transfer uniformly
to learned perceptual distances.
...
metric discussion concrete: the 512-pixel PiD column shows the regular lattice
artifact that led us to exclude it from the quality tables (its valid
native-resolution scores are in \cref{tab:pid_native_quality}), Real-ESRGAN
produces smooth, conservative texture, and \method stays closest to the
reference while still recovering edge detail. The first row is the unfavourable
case, where Real-ESRGAN is closer to the target.
...
\begin{itemize}
  ...
  \item No-reference IQA, FID, and human preference: \todo{run}.
